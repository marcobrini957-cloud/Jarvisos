  ...
  return (
    <>
      ...
      <div className="modal-sheet fixed z-50 vq-r flex flex-col gap-4"
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span style={{ color: 'var(--t1)', fontSize: 'var(--text-md)', fontWeight: 600 }}>{trade.symbol}</span>
            ...
            <p style={{ color: 'var(--t3)', fontSize: 'var(--text-base)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {result === 'breakeven' && (
                      <span style={{ fontSize: 'var(--text-xs)', padding: '1px 6px', borderRadius: 'var(--radius-xs)', background: 'rgba(255,255,255,0.12)', color: 'var(--color-ink-1)', fontWeight: 600 }}>BE</span>
                    )}
                  </>
                )
              })()}
        ...
        <div className="flex flex-col gap-1.5">
          <label style={{ color: 'var(--t2)', fontSize: 'var(--text-base)', fontWeight: 500 }}>Why I took this trade</label>
        ...
        {/* Notes */}
        <div className="flex flex-col gap-1.5">
          <label style={{ color: 'var(--t2)', fontSize: 'var(--text-base)', fontWeight: 500 }}>Notes / Observations</label>
        ...
        <div className="flex flex-col gap-2">
          <label style={{ color: 'var(--t2)', fontSize: 'var(--text-base)', fontWeight: 500 }}>Emotional state before entry</label>
          <div className="flex gap-2 flex-wrap">
            {EMOTIONS.map(em => (
              <button key={em} onClick={() => setEmotion(em === emotion ? '' : em)}
                style={{
                  padding: '5px 12px', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-base)', cursor: 'pointer',
        ...
        <div className="flex items-center gap-3">
          <label style={{ color: 'var(--t2)', fontSize: 'var(--text-base)', fontWeight: 500 }}>Followed trading plan?</label>
          <div className="flex gap-2">
            {[{ val: true, label: 'Yes' }, { val: false, label: 'No' }].map(opt => (
              <button key={String(opt.val)} onClick={() => setFollowed(followed === opt.val ? null : opt.val)}
                style={{
                  padding: '4px 14px', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-base)', cursor: 'pointer',
        ...
        {(trade.screenshot_open_url || trade.screenshot_close_url) && (
          <div className="flex flex-col gap-2">
            <label style={{ color: 'var(--t2)', fontSize: 'var(--text-base)', fontWeight: 500 }}>MT5 Auto Capture</label>
            <div className="flex gap-2">
              {[{ url: trade.screenshot_open_url, label: 'Entry' },
                ...
                <a key={shot.label} href={shot.url} target="_blank" rel="noreferrer"
                  ...
                  <span style={{
                    position: 'absolute', top: '6px', left: '6px', fontSize: 'var(--text-xs)', fontWeight: 700,
                    letterSpacing: '0.05em', padding: '2px 7px', borderRadius: 'var(--radius-sm)',
                    background: 'rgba(0,0,0,0.7)', color: shot.label === 'Entry' ? 'var(--color-surface-3)' : 'var(--am2)',
                  }}>
                    {shot.label}
                  </span>
                </a>
        ...
        {/* Screenshot */}
        <div className="flex flex-col gap-2">
          <label style={{ color: 'var(--t2)', fontSize: 'var(--text-base)', fontWeight: 500 }}>Your Chart Screenshot</label>
          {screenshotUrl ? (
        ...
        {saved && (aiFetching || aiFeedback) && (
          <div style={{
            ...
            <p style={{ fontSize: 'var(--text-xs)', color: 'var(--ac)', letterSpacing: '0.06em', textTransform: 'uppercase', fontWeight: 600, marginBottom: '6px' }}>
              VELQUOR Feedback
            </p>
            {aiFetching ? (
              <p style={{ fontSize: 'var(--text-base)', color: 'var(--t3)' }}>Analyzing trade…</p>
            ) : (
              <p style={{ fontSize: 'var(--text-base)', color: 'var(--t2)', lineHeight: 1.65 }}>{aiFeedback}</p>
            )}
          </div>
        )}
      </div>
