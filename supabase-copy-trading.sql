...
-- ── copy_groups: one "cluster" of leader + follower accounts ──────────────────────
...
-- ── copy_signals: trade signals broadcast by a leader account ─────────────────
CREATE TABLE IF NOT EXISTS copy_signals (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id          UUID NOT NULL REFERENCES copy_groups(id) ON DELETE CASCADE,
  leader_account_id UUID NOT NULL REFERENCES copy_accounts(id),
  signal_type       TEXT NOT NULL CHECK (signal_type IN ('open','close','modify')),
  leader_ticket     BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_copy_log_follower_pending  ON copy_log(follower_account_id, status) WHERE status = 'pending';
