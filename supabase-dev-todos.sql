...
INSERT INTO dev_todos (title, category, done) VALUES
  ...
  ('Update EA (MQL5) — add LEADER / FOLLOWER mode for copy trading', 'copy-trading', false),
  ('Hetzner WebSocket server — add copy group routing (leader → followers)', 'copy-trading', false),
  ('Supabase schema — copy_groups + linked_accounts tables', 'copy-trading', false),
  ('Settings UI — let users designate leader account and add follower accounts', 'copy-trading', false),
  ('Lot sizing config — fixed lots or % of balance per follower', 'copy-trading', false),
